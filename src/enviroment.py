...
class DashCamEnv(core.Env):
    def __init__(self, cfg, device=torch.device("cuda")):
        ...
        self.fps = cfg.FPS / cfg.frame_interval
    ...
    def set_data(self, video_data, data_info):
    ...
    def reset(self):
        ...
        with torch.no_grad():
            self.cur_state = self.observe(frame_data)  # GPU Array(B, 124)
    ...
    def observe(self, data_input):
        ...
        if self.saliency == 'MLNet':
            ...
            saliency = self.minmax_norm(saliency_bu)
            # construct states
            ...
            saliency = self.minmax_norm(saliency_bu.squeeze(2)).unsqueeze(2)
            # construct states
    ...
    def get_reward(self, score_pred):
        ...
        reward_batch = (tta_weights * xnor_dist).unsqueeze(1)
        return reward_batch
    ...
    def step(self, actions, isTraining=True):
        ...
        info = {}
        if not isTraining:
            info.update({'pred_score': score_pred})

        if self.cur_step < self.max_steps - 1:  # cur_step starts from 0
            # next state
            next_state = self.get_next_state(self.cur_step + 1)
            # reward (immediate)
            cur_rewards = self.get_reward(score_pred) if isTraining else 0
        else:
            # The last step
