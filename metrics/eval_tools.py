...
def compute_metrics(preds_eval, all_labels, time_of_accidents, thresolds, fps=30.0):
    ...
    for Th in thresolds:
        ...
        for i in range(len(preds_eval)):
            ...
            if float(len(tp[0])>0) > 0:
                ...
                time += max(0, time_of_accidents[i] - tp[0][0]) / fps  # tta (seconds)
                counter = counter+1
            # all positive frames
...
def evaluation_accident(all_pred, all_labels, time_of_accidents, fps=30.0):
    ...
    Precision, Recall, Time = compute_metrics(preds_eval, all_labels, time_of_accidents, thresolds, fps=fps)
    # when threshold=0.5
    p05, r05, t05 = compute_metrics(preds_eval, all_labels, time_of_accidents, [0.5], fps=fps)
    ...
    return AP, mTTA, TTA_R80, p05[0], r05[0], t05[0]
