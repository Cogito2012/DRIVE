...
from .agents import AccidentPolicy, QNetwork, StateDecoder
from metrics.losses import exp_loss
...
class SAC(object):
    def __init__(self, cfg, device=torch.device("cuda")):
        ...
        self.policy_accident, self.critic, self.critic_target = self.create_actor_critics(cfg)
        ...
        if self.automatic_entropy_tuning and self.type_acc == "Gaussian":
            # Target Entropy = −dim(A) (e.g. , -6 for HalfCheetah-v2) as given in the paper
            self.target_entropy = -self.dim_action
    ...
    def create_actor_critics(self, cfg):
        ...
        policy_accident = AccidentPolicy(self.dim_state, self.dim_action, cfg.hidden_size, 
            dim_latent=cfg.dim_latent, arch_type=cfg.arch_type, policy_type=self.type_acc).to(self.device)
        return policy_accident, critic, critic_target
    ...
    def select_action(self, state, rnn_state=None, evaluate=False):
        ...
        if evaluate is False:
            actions, rnn_state, _, _ = self.policy_accident.sample(state, rnn_state)
        else:
            _, rnn_state, _, actions = self.policy_accident.sample(state, rnn_state)
        if rnn_state is not None:
            rnn_state = (rnn_state[0].detach(), rnn_state[1].detach())
        return actions.detach(), rnn_state
    ...
    def update_critic(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch, rnn_state_batch):
        with torch.no_grad():
            ...
            next_state_action, _, next_state_log_pi, _ = self.policy_accident.sample(next_state_batch, rnn_state_batch)
            # interence critics
    ...
    def update_actor(self, state_batch, rnn_state_batch, labels_batch):
        # sampling
        pi, _, log_pi, mean = self.policy_accident.sample(state_batch, rnn_state_batch, detach=True)
        ...
        score_pred = 0.5 * (mean + 1.0).squeeze(1)  # (B,)
    ...
    def save_models(self, ckpt_dir, cfg, epoch):
        model_dict = {'policy_model': self.policy_accident.state_dict(),
                      'configs': cfg}
    ...
    def load_models(self, ckpt_dir, cfg):
        ...
        if os.path.isfile(weight_file):
            ...
            self.policy_accident.load_state_dict(checkpoint['policy_model'])
